...
describe('GET /signup', () => {
  ...
  const fs = {
    readFile: () => { },
    readFileSync: mockReadFileSync('./src/app/template/signup.html', 'Signup Template', 'utf8')
  };
...
describe('POST /sign-up', () => {
  const appConfig = { staticDir: './public', usersDataPath: './data/userDetails.json' };
  const users = {
  ...
  const fs = {
    readFileSync: mockReadFileSync('./data/userDetails.json', JSON.stringify(users), 'utf8'),
    writeFileSync: mockWriteFileSync('./data/userDetails.json', JSON.stringify(updatedUsers), 'utf8')
  };
